...
const props = defineProps<{
  ...
  isPending: boolean;
  mutationUpdate: any;
  miembros: { nombreCompleto: string; idmiembro: number }[];
  refetch: any;
}>();
...
<template>
  ...
  <DataTable
    :class="{
      'skeleton select-none': props.isPending,
      'cursor-wait select-none': props.mutationUpdate.isPending.value,
    }"
    v-else
    :value="props.data"
    :paginator="true"
    ...
    <Column field="director" header="Director(a) responsable del proyecto">
      ...
      <template #editor="{ data, field }">
        ...
        >
          ...
          <option v-for="miembro in props.miembros" :value="miembro.idmiembro">
    ...
    <Column field="miembrosCol" header="Miembros colaboradores">
      ...
      >
        ...
        <MultiSelect
          ...
          @vue:mounted="colaboradores = data[field]"
          :options="miembros"
          option-label="label"
          option-value="value"
          display="chip"
